...
export default abstract class Vault {
	...
	async getPaymentMethodForThisSystem(
		...
		userPaymentMethodTable: IUserPaymentMethodTable
		...
		const paymentMatches: Model.UserPaymentMethod[] = await userPaymentMethodTable.getByProperties(
			cardByPropertiesRequest
		);
